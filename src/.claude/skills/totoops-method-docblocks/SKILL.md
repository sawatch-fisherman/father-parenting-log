...
背景として書いてよいのは、**いまこの形になっている仕様・設計上の理由**だけ。「PR #N レビュー指摘」「〜の指摘で修正した」「以前は〜だった」といった**経緯は書かない**（本システムは未リリースで、いまのコードが常に正しい仕様。`CLAUDE.md`「リリース状態（未リリース）」を参照）。この規約は PHP に限らず、Vue・TypeScript 側のコメントにも同じように適用する。
