...
本システムは未リリースで、守るべき本番の挙動が存在しない（`CLAUDE.md`「リリース状態（未リリース）」）。テストではこれを次のように適用する。

- **仕様変更で既存テストが落ちたら、新しい仕様に合わせてテスト側を書き換える。** 旧仕様を守るテストを残さない。新旧どちらでも通るようにアサーションを緩めることもしない。
- **後方互換の確認テストを追加しない。** 「変更前の挙動も引き続き通ること」を目的にしたテスト、旧挙動用のリグレッションテストは書かない。
- **docblock にはその時点の仕様だけを書く。** 「〜のレビュー指摘で追加」「以前は〜だった」のような経緯は書かず、過去を知らない読み手がそのまま読める状態にする。不具合を直したときも「その不具合の再現」ではなく「正しい仕様の検証」として書く。
- **ただし、仕様として意味のある境界値・異常系・失敗系のテストは通常どおり必要。** 「未リリースだからテストを書かなくてよい」ではない。落ちたテストを削除・スキップで済ませることもしない。
