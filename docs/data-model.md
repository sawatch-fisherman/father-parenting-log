...
- `Streak`は専用の集計テーブル・カラムを持たず、**都度動的に計算**する：判定対象範囲（`care_event_type_id`がNULLなら全種別、値ありならその種別のみ）で`care_events.occurred_at`のDISTINCTな日付（JST暦日）を新しい順に取得し、直近保存したイベントの日付を起点に連続日数を数える（[decisions.md](decisions.md) §1.3）。
