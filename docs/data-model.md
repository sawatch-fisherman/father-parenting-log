...
- **`sort_order`の採番規則**：`TitleSeeder`の配列の並び順をそのまま`1`から採番する。並び順は次の**4段階**で決まり、育児行動別の2ブロックはいずれも`care_actions.sort_order`（育児行動の表示順）に揃える（テスト`TitleSeedIntegrityTest`で検証する）。
  ...
  - **全体称号を先頭に置く理由**：図鑑は90件の一覧で全体称号はうち6件しかなく、育児行動別84件の後ろに置くと発見性が失われる（総論→各論）。加えて図鑑は「獲得済み＋各系統の未獲得最小しきい値1件」方式のため、登録直後のユーザーが最初に見る目標が「育児見習い（合計100回）」「1週間連続育児ログ」という**全ユーザー共通のもの**になる（先頭が育児行動別だと、子どもが3歳の父親に「おむつ交換見習い」が最初に出るような的外れが起きる）。S5の獲得モーダルは同時獲得が1〜3件で先頭/末尾の差が効きにくいため、`sort_order`は図鑑の見やすさを正とする。モーダルだけ「全体を締めに出す」演出が必要になったら、レスポンス組み立て側の並べ替えで対応する。
