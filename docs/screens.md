...
> [features.md](features.md) のMVP機能一覧・[data-model.md](data-model.md) のドメイン7テーブルをもとに、実際の画面単位に落とし込みます。
...
| S2 | プロフィール登録画面 | 初回ログイン時のみ。ニックネーム（必須）・年代／子どもの年齢帯（末子1つのみ。ともに任意） | プロフィール登録 |
...
| S11 | ログ編集画面 | S13（記録履歴画面）の各行の「・・・」から遷移する。**「実施日時・メモの変更」または「削除」のみ**可能（育児行動の変更は不可。育児行動を変えたい場合は削除→S3/S4から再記録）。**`occurred_at`が「7日前の00:00」より前の記録は変更・削除とも不可**（S13側で導線を非活性にする） | 育児ログ登録 |
...
| S13 | 記録履歴画面（タイムライン） | 日付ごとにグループ化したログ一覧（新しい順）。各行「・・・」→S11。**`occurred_at`が「7日前の00:00」より前の記録は「・・・」を非活性にし、編集・削除できない理由を添える** | 記録履歴（タイムライン） |

Phase 2以降の「全体タスク傾向」「卒業＆エンドロール」「称号一覧（図鑑）画面」等の画面はこの一覧には含めていません（MVP対象外）。

**S6は欠番**：かつて「X投稿文生成モーダル」をS6として別に置いていたが、S5に統合して廃止した（[decisions.md](decisions.md) §1.3）。S7以降の番号は繰り上げず、S6を欠番のまま残す（他ドキュメント・コード注釈からの参照を無効にしないため）。

---

## 共通UI：グローバルナビゲーション

S3・S12・S13・S7の4画面は、常時表示のナビゲーションから互いに行き来する。それ以外の画面（S4・S10・S11・S2・S8・S9）やモーダル（S5）には表示しない。
...
- S4・S10・S11は入力・編集を伴うため独立したページ（URLを持つ画面）とする。S5は表示のみのためモーダル（S3上のUI状態）のまま、専用ルートは持たない。詳細は次の「ルーティング一覧」を参照。
- S5の「Xに投稿」はXの投稿画面（`https://x.com/intent/post?text=...`）を別タブで開く**外部遷移**のため、上図には含めない。モーダル自体は開いたままで、戻ってきたら「閉じる」でS3に戻る（同時獲得が複数ある場合は次の称号のS5が続けて出る）。
...
**ルートを持たない画面**：S5（称号獲得モーダル）。`POST /care-logs`のレスポンス（称号獲得情報を含む）を受けてVue側で自動表示する。X投稿文も同じレスポンスに含まれる`achievement_text`（達成内容を表す一文。サーバー側で組み立て済み。[decisions.md](decisions.md) §1.3）を、絵文字・称号名・ハッシュタグ等の固定レイアウトにクライアント側で埋め込んで組み立てるだけなので、追加のサーバー往復は不要。
...
| S11 ログ編集画面 | GET `/care-logs/{care_log}/edit` | `CareLogController` | `edit()` | — | `CareLogPolicy@update`（他人の記録IDを弾く／`occurred_at`が「7日前の00:00」より前の記録を弾く） |
...
- `ProfileRequest`はstore/updateで同一ルール（`nickname`必須、`age_group`/`child_age_group`任意）のため1クラスを共用する。`child_age_group`は末子（いちばん下の子）の年齢帯を表す単一の値で、子どもが複数いる場合も配列にはしない（[decisions.md](decisions.md) §1.1）。
